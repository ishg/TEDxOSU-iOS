...
class SpeakerModel: NSObject {
    ...
    let jsonObject: [AnyObject] = [
        ...
        [
            ...
            "lines":"Kawayan's performance is a combination of the Filipino Martial Art, Arnis, and a modern interpretation of the Filipino warrior spirit through the use of modern hip-hop beats and bamboo sticks.",
            "speech": "Kawayan: Spirit of The Warrior"
        ]
    ]
    ...
    func getSpeakers() -> [Speaker]{
        ...
        for (_,s):(String, JSON) in json {
            ...
            if let title = s["title"].string {
                speaker.title = title
            }
            if let bio = s["lines"].string {
                speaker.bio = bio
            }
            if let photo = s["picture"].string {
                speaker.photo = photo
            }
            if let speech = s["speech"].string {
    ...
    let schedule = [
        ["kawayan", "matheny", "debanuj", "bailo", "chess","joshua","sutter"],
        ["donnermeyer", "cynthia", "austin", "kristina", "abd"],
        ["jonathon", "bria", "moore", "carlarne", "richardson", "brehm"]
    ]
    
    func getSession(session: Int) -> [Speaker]{
        let json = JSON(jsonObject)
        let scheduled = schedule[session-1]
        var speakers = [Speaker]()
